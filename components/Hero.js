...
const IndexRow = ({ label, children }) => (
	<section className="grid gap-4 border-t border-white/10 py-8 sm:grid-cols-[8rem_1fr] sm:gap-8 sm:py-10" data-reveal-group>
		<h2 className="text-sm font-medium text-neutral-100" data-reveal-item>{label}</h2>
...
const Hero = () => (
	<div className="container py-12 sm:py-16">
		<header className="grid gap-7 pb-12 sm:grid-cols-[1fr_8rem] sm:items-start sm:gap-10 sm:pb-16" data-reveal-group>
			<div className="grid gap-5" data-reveal-item data-reveal-group>
				<div className="grid gap-1" data-reveal-item>
				...
				<p className="max-w-[34ch] text-2xl tracking-tight text-pretty text-neutral-300 sm:text-3xl" data-reveal-item>
					I turn unclear ideas and complicated systems into useful digital products.
				</p>
				<div className="flex flex-wrap gap-x-6 gap-y-3 text-sm" data-reveal-item>
			...
			<div className="relative row-start-1 aspect-[4/5] w-24 overflow-hidden sm:col-start-2 sm:w-32" data-reveal-item="image">
		...
		<IndexRow label="Focus">
			<ul role="list" className="grid" data-reveal-group>
				{capabilities.map(([title, text]) => (
					<li key={title} className="grid gap-1 border-t border-white/10 py-4 first:border-t-0 first:pt-0 sm:grid-cols-[10rem_1fr] sm:gap-5" data-reveal-item>
		...
		<IndexRow label="Current">
			<dl className="grid" data-reveal-group>
				{currentWork.map(([name, description]) => (
					<div key={name} className="grid gap-1 border-t border-white/10 py-4 first:border-t-0 first:pt-0 sm:grid-cols-[8rem_1fr] sm:gap-5" data-reveal-item>
		...
		<section aria-label="Personal photographs" className="grid gap-3 border-t border-white/10 py-8 sm:grid-cols-2 sm:py-10" data-reveal-group>
